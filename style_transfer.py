...
class StyleLossForImage(nn.Module):
    def __init__(self, target_feature):
        super(StyleLossForImage, self).__init__()
...
class Simple_style_transfer:
    def __init__(self, style_img, content_img, imgsize=1024, num_steps=500,
    ...
    def get_style_model_and_losses(self):
        ...
        for layer in cnn.children():
            ...
            if name in self.style_layers:
                ...
                style_loss = StyleLossForImage(target_feature)
                model.add_module("style_loss_{}".format(i), style_loss)
                style_losses.append(style_loss)

        for i in range(len(model) - 1, -1, -1):
            if isinstance(model[i], ContentLoss) or isinstance(model[i], StyleLossForImage):
                break
    ...
    async def transfer(self):
        ...
        if CNN == '':
            CNN = torch.load('models_pretrained/vgg19.pth', ).to(self.device).eval()
